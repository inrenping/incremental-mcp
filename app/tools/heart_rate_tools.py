...
def get_heart_rate_history(days: int = 7) -> dict:
    """获取当前用户最近 N 天的每日心率汇总（按日期倒序）。

    Args:
        days: 返回最近多少天的记录，默认 7。
    ...
    with SessionLocal() as session:
        result = session.execute(
            ...
            .where(HeartRateDaily.user_id == int(user_id))
            .order_by(HeartRateDaily.calendar_date.desc())
            .limit(max(days, 0))
        )
...
def get_daily_heart_rate(date: str | None = None) -> dict:
    ...
    with SessionLocal() as session:
        ...
        start_of_day = datetime.combine(
            query_date, time.min, tzinfo=tz
        ).astimezone(timezone.utc)
        end_of_day = datetime.combine(
            query_date, time.max, tzinfo=tz
        ).astimezone(timezone.utc)
